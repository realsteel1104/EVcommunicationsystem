...
def publish_data(text,text1,deviation_text):
    ...
    client.publish("Lane Detection Parameters", text,qos=1)
    print("Just published " + str(text) + " to topic Lane Detection")
    client.publish("Lane Detection Parameters", text1)
    print("Just published " + str(text1) + " to topic Lane Detection")
    client.publish("Lane Detection Parameters", deviation_text)
    print("Just published " + str(deviation_text) + " to topic Lane Detection")
    print()
    #time.sleep(1)
...
def addText(img, radius, direction, deviation, devDirection):
    ...
    cv2.putText(img, text , (50,50), font, 0.9, (0,0,0), 5, cv2.LINE_AA)
    cv2.putText(img, text , (50,50), font, 0.9, (255,255,255), 2, cv2.LINE_AA)
    cv2.putText(img, text1, (50,100), font, 0.9, (0,0,0), 5, cv2.LINE_AA)
    cv2.putText(img, text1, (50,100), font, 0.9, (255,255,255), 2, cv2.LINE_AA)
    # Deviation
    deviation_text = 'Off Center: ' + str(round(abs(deviation), 3)) + 'm' + ' to the ' + devDirection
    cv2.putText(img, deviation_text, (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,0), 5, cv2.LINE_AA)
    cv2.putText(img, deviation_text, (50, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255,255,255), 2, cv2.LINE_AA)
    #Publish Data
    publish_data(text,text1,deviation_text)
    return img
...
image = readVideo()
fields=['Date-Time','Radius of Curvature (m)','Curve Direction','Off Center Distance (m)','Deviated Direction']
filename = "Lane Detection Data.csv" 
with open(filename, 'a') as csvfile:  
    csvwriter = csv.writer(csvfile)    
    csvwriter.writerow(fields) 

def write_data(curveRad, curveDir, deviation, directionDev):
    row=[datetime.datetime.now(),curveRad, curveDir, round(abs(deviation), 3), directionDev]
    filename = "Lane Detection Data.csv" 
    with open(filename, 'a') as csvfile:  
        csvwriter = csv.writer(csvfile)    
...
while True:
    ...
    #Write Data
    write_data(curveRad, curveDir, deviation, directionDev)
